...
public class BukuDAO {
    ...
    public static int save(Buku b) throws ClassNotFoundException {
        ...
        try {
            ...
            ps = conn.prepareStatement("INSERT INTO `pustakakita1`.`tbl_buku` (`id_buku`, `judul`, `penulis`, `penerbit`, `harga`, `stok`) VALUES (0,?,?,?,?,?);");
    ...
    public static int update(Buku b) throws ClassNotFoundException {
        ...
        try {
            conn = new DBConnection().setConnection();
            ps = conn.prepareStatement("update pustakakita1.tbl_buku set judul_buku=?,penulis=?,penerbit=?,harga=?,stok=? where id_buku=?");
            ps.setString(1, b.getJudul());
            ps.setString(2, b.getPenulis());
            ps.setString(3, b.getDeskripsi());
            ps.setDouble(4, b.getHarga());
    ...
    public static int delete(Buku b) throws ClassNotFoundException {
        ...
        try {
            conn = new DBConnection().setConnection();
            ps = conn.prepareStatement("delete from pustakakita1.tbl_buku where id_buku=?");
            ps.setInt(1, b.getId());
            status = ps.executeUpdate();
        } catch (Exception e) {
            System.out.println(e);
    ...
    public static List<Buku> getAllRecords() throws ClassNotFoundException {
        ...
        try {
            conn = new DBConnection().setConnection();
            ps = conn.prepareStatement("select * from pustakakita1.tbl_buku");
    ...
    public static Buku getRecordById(int id) throws ClassNotFoundException {
        ...
        try {
            conn = new DBConnection().setConnection();
            ps = conn.prepareStatement("select * from pustakakita1.tbl_buku where id_buku=?");
            ps.setInt(1, id);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                u = new Buku();
